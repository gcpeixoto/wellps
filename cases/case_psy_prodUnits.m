...
minm = mean(RQI_SO_norm); % lim inf by mean
...
nclasses = 3;
div = linspace(minm,1,nclasses+1); % divisions for nclasses
...
for f = 1:nn % loop PUC
    ...
    for fc = 1:length(fn) % loop clusters
        ...
        fprintf('%s\n',repmat('=',[1,75]))
        outfile2 = fullfile(d.getTmpDir,'betNX.txt');        
        exec2 = [d.getPyExec,' ',d.getPyDir,filesep, ...
                sprintf('graphMetricsN.py %s %s',edfile,outfile2)];
        system(exec2);        
        % get data
        tab2 = importdata(outfile2);                       
        Mtab2 = tab2.data(:,1:4);            
        nodeID2 = Mtab2(:,1);        
