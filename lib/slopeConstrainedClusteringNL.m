function [clusters,R2,M,B] = slopeConstrainedClusteringNL(X,Y,i0,seps)
...
%   in normal direction to straight line (Condition NL: NORMAL TO
%   (CENTER)LINE).
%
%   Note that r+ has slope 1 and crosses the point (X(i),Y+), 
%   whereas   r- has slope 1 and crosses the point (X(i),Y-), 
%   in that 
%   Y+ = X(j) + Y(i) - X(i) + 2*seps/sqrt(2)
%   Y- = X(j) + Y(i) - X(i) - 2*seps/sqrt(2)
%   
%   SEE wellps::slopeConstrainedClusteringPY
%
%
...
while numel(I) > 0 % while there exists free indices
    ...
    for j = I                    
        ...
        if X(j) + Y(i) - X(i) - 2*seps/sqrt(2) <= Y(j) && Y(j) <= X(j) + Y(i) - X(i) + 2*seps/sqrt(2)
