function Mf = computePUCGraphMetrics(opt_metrics, pucSt)
%computePUCGraphMetrics compute metrics for networks based 
%                       on PUC-values by using a wrap to SNAP package.
%
% PARAMETERS: 
...
if ~all(ismember(p,fieldnames(opt_metrics)))   
    error('wellps:computePUCGraphMetrics','opt_metrics is not a struct object');
end
    
% standard output dir
if isempty(find(ismember(fieldnames(opt_metrics),'outDir'), 1))     
    warning('wellps:computePUCGraphMetrics','Output directory was set to standard');
    outDir = matdir;
else 
