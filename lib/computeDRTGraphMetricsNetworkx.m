function [Mf,Lf] = computeDRTGraphMetricsNetworkx(opt_metrics, drtSt)
%computeDRTGraphMetricsNetworkx compute metrics and regression analysis for 
%                       the whole field's networks based on DRT 
%                       by using a wrap to NETWORKX package.
%
% PARAMETERS: 
...
if ~all(ismember(p,fieldnames(opt_metrics)))   
    error('wellps:computeDRTGraphMetricsNetworkx','opt_metrics is not a struct object');
end
    
% standard output dir
if isempty(find(ismember(fieldnames(opt_metrics),'outDir'), 1))     
    warning('wellps:computeDRTGraphMetricsNetworkx','Output directory was set to standard');
    outDir = matdir;
else 
...
for n = 1:nnames
    ...
    if count ~= 0 % saving structure to .mat, if any         
        ...
        fprintf('=> computeDRTGraphMetricsNetworkx finished after %g seconds.\n',toc(tstart));
end
